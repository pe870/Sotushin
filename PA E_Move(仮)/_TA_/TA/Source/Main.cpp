...
void clsMain::AppMain()
{
	...
	//{
	//	m_pPlayer->m_vPos.x = PlayerX;
	//	m_pPlayer->m_vPos.y = PlayerY;
	//	m_pPlayer->m_vPos.z = PlayerZ;
	//}
	...
	//if (E_moveCnt < 100)
	//	m_aEnemy->m_vPos.x += 0.03f;
	//if (101 <E_moveCnt && E_moveCnt < 170)
	//	m_aEnemy->m_vPos.x += 0.0f;
	...
	case PAT:
		...
		for (int i = 0; i < E_MAX; i++)
		{
			if (i % 2 == 0)
			{
				...
				if (271 < E_moveCnt && E_moveCnt < 328)
				{
					m_aEnemy[i]->m_vPos.x -= 0.0f;
					m_aEnemy[i]->m_Yaw -= 0.045f;
		...
		/*2つのパターンの⑴行動後の初期化処理*/
			if (329 < E_moveCnt)
			{
				if (!TRIP_E && !SEARTCH_E)	//自機が周りにいないか.
				{
...
void clsMain::LoadModel()
{
	...
	m_pPlayer->Init(m_hWnd, m_pDevice, m_pContext, "Data\\Model\\Player\\player2.x");
	...
	for (int i = 0; i < E_MAX; i++)
	{
		...
		m_aEnemy[i]->m_vPos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);

	}
	m_aEnemy[0]->m_vPos = D3DXVECTOR3(PlayerX, PlayerY, PlayerZ+1.0);
	m_aEnemy[1]->m_vPos = D3DXVECTOR3(PlayerX, PlayerY, PlayerZ+2.0);
	m_aEnemy[2]->m_vPos = D3DXVECTOR3(PlayerX, PlayerY, PlayerZ+3.0);
	m_aEnemy[3]->m_vPos = D3DXVECTOR3(PlayerX, PlayerY, PlayerZ+4.0);
	m_aEnemy[4]->m_vPos = D3DXVECTOR3(PlayerX, PlayerY, PlayerZ + 5.0);
