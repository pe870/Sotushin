...
//判定関係(雑魚).
bool KnkBackSX[E_MAX], KnkBackBX[E_MAX];
bool KnkBackSZ[E_MAX], KnkBackBZ[E_MAX];
...
void clsMain::AppMain()
{
	...
	for (Ei = 0; Ei < E_MAX; Ei++)
	{
		if (Enemy->E_ATTACKX[Ei]
			&& BBoxCollision(Enemy->m_aEnemy[Ei], Player->m_pPlayer)
			&&!Player->invicible_P)
		{
			Knkback = 1.5;
			...
			if (Enemy->m_aEnemy[Ei]->m_vPos.x < Player->m_pPlayer->m_vPos.x)
			{
				KnkBackBX[Ei] = true;

			}
			else if (Enemy->m_aEnemy[Ei]->m_vPos.x > Player->m_pPlayer->m_vPos.x)
			{
				KnkBackSX[Ei] = true;

			}

			if (Enemy->m_aEnemy[Ei]->m_vPos.z < Player->m_pPlayer->m_vPos.z)
			{
				KnkBackBZ[Ei] = true;

			}
			else if (Enemy->m_aEnemy[Ei]->m_vPos.z > Player->m_pPlayer->m_vPos.z)
			{
				KnkBackSZ[Ei] = true;
			}
	...
	for (Ei = 0; Ei < E_MAX; Ei++)
	{
		...
		{
			Knkback -= 0.02;
			Player->m_pPlayer->m_vPos.x += 0.1*Knkback;
			if (Knkback < 0.0f)
			{
				Knkback = 1.0f;
				Player->invicible_P = true;
	...
	if (Player->invicible_P)
	{
		InvCnt++;
		if (InvCnt > 100)
		{
			InvCnt = 0;
			Player->invicible_P = false;
		}
	}
...
void clsMain::E_CollMove()
{
	...
	for (Ei = 0; Ei < E_MAX; Ei++)
	{
		...
		if (Enemy->E_ATTACKX[Ei])
		{
			...
			Enemy->E_SearchMoveCnt[Ei]++;

			if (Enemy->E_SearchMoveCnt[Ei] > 0 && Enemy->E_SearchMoveCnt[Ei] <= 500)
			{
			...
			//追尾をやめさせその場で索敵再開
			if (Enemy->E_SearchMoveCnt[Ei] >= 550)
			{

				if (Enemy->E_SearchMoveCnt[Ei] > 600)
				{
					Enemy->E_SearchMoveCnt[Ei] = 0;
					Enemy->SEARCH_EX[Ei] = false;
					Enemy->SEARCH_EZ[Ei] = false;
					Enemy->E_ATTACKZ[Ei] = false;
					Enemy->E_ATTACKX[Ei] = false;
...
void clsMain::E_SearchColl()
{
	...
	for (Ei = 0; Ei < E_MAX; Ei++)
	{
		if (Ei % 2 == 0)
		{
			...
			{
				Enemy->SEARCH_EX[Ei] = true;
				Enemy->E_ATTACKX[Ei] = true;
			}
		...
		{
			if (Collision(Player->m_pPlayer, Enemy->m_aEnemySearch[Ei])
				|| Collision(Enemy->m_aEnemySearch[Ei], Player->m_pPlayer))
			{
				Enemy->SEARCH_EZ[Ei] = true;
				Enemy->E_ATTACKZ[Ei] = true;
			}
