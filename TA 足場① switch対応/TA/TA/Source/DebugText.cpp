	...
	float Size, D3DXVECTOR4 vColor)
{
	...
	{
		MessageBox(NULL, "ﾃﾞﾊﾞｯｸﾞﾌｫﾝﾄﾃｸｽﾁｬ作成失敗", "error", MB_OK);
		return E_FAIL;
	}
	...
	{
		MessageBox(NULL, "ﾌﾞﾛﾌﾞ作成失敗(vs)", "error", MB_OK);
		return E_FAIL;
	}
	...
	if (FAILED(
		...
		0, 0, NULL, &pCompileShader, &pErrors, NULL)))
	{
		MessageBox(NULL, "ﾌﾞﾛﾌﾞ作成失敗(ps)", "error", MB_OK);
		return E_FAIL;
	}
	SAFE_RELEASE(pErrors);
	...
	{
		MessageBox(NULL, "ｺﾝｽﾀﾝﾄﾊﾞｯﾌｧ作成失敗", "error", MB_OK);
		return E_FAIL;
	}
