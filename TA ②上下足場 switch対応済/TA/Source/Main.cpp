...
void clsMain::Render()
{
	...
	*/
	...
	sprintf_s(strDebugText, "dY[%f] dY[%f]", m_aScafUp->m_vPos.y, m_aScafDown->m_vPos.y);
	//テキスト描画.
...
void clsMain::LoadModel()
{
	...
	m_aSaw->m_vPos = D3DXVECTOR3(ObstacleX1, ObstacleY1, ObstacleZ1);
	m_aSaw->m_Pitch = 80.1f;
	m_aSaw->m_Scale = 0.6f;

	m_aSawCover = new clsDx9Mesh;
	m_aSawCover->Init(m_hWnd, m_pDevice, m_pContext, "Data\\Model\\Obstacle\\cover.x");
	m_aSawCover->m_vPos = D3DXVECTOR3(ObstacleX1-0.5, ObstacleY1-0.85f, ObstacleZ1-1.45f);
...
void clsMain::SawControll()
{
	...
	case MARK1:
		...
		if (300 < SawControllCnt)
		{
			m_aSaw->m_vPos.x = 3.57;//3.6
			Germany = MARK2;
		}
	...
	case MARK2:
		m_aSaw->m_vPos.x -= 0.023f;
		if (605 < SawControllCnt)
		{
			m_aSaw->m_vPos.x = -3.57f;
			SawControllCnt = 0;
			Germany = MARK1;
		}
		break;
