...
void clsMain::AppMain()
{
	...
	bool gHit2 = false;
	bool gHit3 = false;
	bHit = Intersect(m_pPlayer, m_pGround, &fDistance, &vIntersect);

	gHit = Intersect(m_pPlayer, m_aGround[0], &fDistance, &vIntersect);
	gHit2 = Intersect(m_pPlayer, m_aGround[1], &fDistance, &vIntersect);
	gHit3 = Intersect(m_pPlayer, m_aGround[2], &fDistance, &vIntersect);

	if (bHit == true || gHit == true || gHit2 == true || gHit3 ==true)
	{
		SetWindowText(m_hWnd, "on the 足場");
	}
	else
	{
...
void clsMain::Render()
{
	...
	//Rayのレンダリング（垂直）.
	//位置を自機に合わせる.
	m_pRayV->m_Ray.vPos = m_pPlayer->m_vPos;
	//回転軸を自機に合わせる.
...
/****************読込関数コーナー******************/
	...
	void clsMain::LoadModel()
	{
		...
		m_pPlayer->m_vPos = D3DXVECTOR3(PlayerX, PlayerY, PlayerZ-2.0f);
		...
		m_pRayV->Init(m_pDevice, m_pContext);
