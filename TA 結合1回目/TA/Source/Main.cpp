...
//m_pClsDx9Mesh->Render(mView, mProj, m_vLight, m_vCamera);
	...
	if (!ColorChange)	m_aScaf_Paste->Render(m_mView, m_mProj, m_vLight, m_Camera.vEye);
	if (ColorChange)	m_aScaf_PasteOn->Render(m_mView, m_mProj, m_vLight, m_Camera.vEye);
...
void clsMain::P_ACT()
{
	...
	if (m_acc.y < -0.05f) m_acc.y = -0.05f;
	if (m_acc.z >  0.05f) m_acc.z = 0.05f;
	if (m_acc.z < -0.10f) m_acc.z = -0.10f;

	if (m_pPlayer->m_vPos.x > -4.0f)
	{
		if (m_pPlayer->m_vPos.x > 8.0f)
		{
			if (m_pPlayer->m_vPos.z > -10.0f)
			{
				if (m_pPlayer->m_vPos.z > 10.0f)
				{
					if (m_pPlayer->m_vPos.y <= 1.0f)
					{
						m_pPlayer->m_vPos.y = 1.0f;
					}
				}
			}
		}
