	...
	E_moveCnt++;
		...
		{	
			if (!TRIP_E[i] && !SEARCH_E[i])
			{
				...
				if (i % 2 == 1)
				{
					if (Collision(m_pPlayer, m_aEnemySearch[i]))	//自機攻撃条件.
					{
						TRIP_E[i] = true;
					}
			...
			if (SEARCH_E[i])
			{
				...
				{
					if (m_aEnemy[i]->m_vPos.z < m_pPlayer->m_vPos.z)
					{
						m_aEnemy[i]->m_vPos.z += 0.02;

					}
					else if (m_aEnemy[i]->m_vPos.z > m_pPlayer->m_vPos.z)
					{
						m_aEnemy[i]->m_vPos.z -= 0.02;

					}

					if (m_aEnemy[i]->m_vPos.x < m_pPlayer->m_vPos.x)
					{
						m_aEnemy[i]->m_vPos.x += 0.03;
					}
					else if (m_aEnemy[i]->m_vPos.x > m_pPlayer->m_vPos.x)
					{
						m_aEnemy[i]->m_vPos.x -= 0.03;
					}
