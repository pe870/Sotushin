...
HRESULT clsRay::InitModel()
{
	...
	bd.ByteWidth = sizeof(RAY_VERTEX) * 2;
	bd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bd.CPUAccessFlags = 0;
	bd.MiscFlags = 0;

	D3D11_SUBRESOURCE_DATA InitData;
	InitData.pSysMem = m_Ray.vPoint;	//レイの座標のセット.
	if (FAILED(m_pDevice11->CreateBuffer(
		&bd, &InitData, &m_pVertexBuffer)))
	{
		ERR_MSG("頂点バッファの作成失敗", "error");
		return E_FAIL;
	}
	return S_OK;
}
...
void clsRay::Render(D3DXMATRIX &mView, D3DXMATRIX &mProj)
{
	...
	//平行移動.
	D3DXMatrixTranslation(&mTran, m_Ray.vPos.x, m_Ray.vPos.y, m_Ray.vPos.z);
	//ワールド行列作成.
	mWorld = mRot * mTran;
