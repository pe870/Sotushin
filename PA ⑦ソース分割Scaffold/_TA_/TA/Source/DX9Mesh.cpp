...
HRESULT clsDx9Mesh::Init(HWND hWnd, ID3D11Device* pDevice11,ID3D11DeviceContext* pContext11, LPSTR fileName)
{
	...
	{
		MessageBox(NULL, "Xﾌｧｲﾙ読込失敗222", "error", MB_OK);
		return E_FAIL;
	}
...
void clsDx9Mesh::Render(D3DXMATRIX &mView, D3DXMATRIX &mProj,D3DXVECTOR3 &vLight, D3DXVECTOR3 &vEye)
{
	...
	switch (m_eDir)
	{
	case enDirectioin_Forward:	//前進.
		//向かう方角*進める値（0.1ｆ）
		m_vPos += vecAxisZ*0.1;
		break;
	case enDirectioin_Backward:	//前進.
		//向かう方角*進める値（0.1ｆ）
		m_vPos -= vecAxisZ*0.1;
		break;
	case enDirectioin_Left:
		m_vPos -= vecAxisX*0.1;
		break;
	case enDirectioin_Right:
		m_vPos += vecAxisX*0.1;
		break;


	}
	m_eDir = enDirectioin_Stop;	//停止.
